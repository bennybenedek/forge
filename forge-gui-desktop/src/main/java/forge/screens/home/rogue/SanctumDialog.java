...
public class SanctumDialog {
    ...
    public SanctumDialog(int currentLife, int maxLife, int healAmount, int freeRemoves) {
        ...
        btnHeal.setIcon(FSkin.getIcon(FSkinProp.ICO_QUEST_LIFE));
        ...
        // Remove cards button
        btnRemove = new FButton("Remove up to " + freeRemoves + " Cards from Deck");
        btnRemove.setIcon(FSkin.getIcon(FSkinProp.ICO_DELETE));
        ...
        panel.add(lblTitle, "w 100%!, h 60px!, gap 0 0 20px 10px, wrap");
        panel.add(lblDescription, "w 100%!, h 30px!, gap 0 0 10px 20px, wrap");
    ...
    private class MainPanel extends SkinnedPanel {
        private MainPanel() {
            super(new MigLayout("insets 20, gap 0, wrap", "[grow]", ""));
            setOpaque(false);
        }
    }
}
