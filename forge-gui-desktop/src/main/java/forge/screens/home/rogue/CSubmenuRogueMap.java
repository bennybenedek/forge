...
public enum CSubmenuRogueMap implements ICDoc {
    ...
    /**
     * Load the most recent continuable run from saved files.
     * A continuable run is one that is in STARTED state (not WON or LOST).
    ...
    /**
     * Extract timestamp from run filename.
     * Format: {DeckName}_{timestamp}
     */
